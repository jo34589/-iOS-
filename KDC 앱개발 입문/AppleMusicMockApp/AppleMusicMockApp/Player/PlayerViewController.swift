...
extension PlayerViewController {
    ...
    func updateTime(time: CMTime) {
        ...
        //[x] TODO: 시간정보 업데이트, 심플플레이어 이용해서 수정
    ...
    func updatePlayButton() {
        ...
        let conf = UIImage.SymbolConfiguration(pointSize: 40)
        if simplePlayer.isPlaying {
            let image = UIImage(systemName: "pause.fill", withConfiguration: conf)
            playControlButton.setImage(image, for: .normal)
        } else {
            let image = UIImage(systemName: "play.fill", withConfiguration: conf)
            playControlButton.setImage(image, for: .normal)
        }
    }
}
